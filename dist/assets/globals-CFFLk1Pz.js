...
const useProfileStore = create((set, get) => ({
  ...
  get activeProfile() {
    ...
    return profiles.find((p2) => p2.id === activeProfileId) ?? null;
  },
  get defaultProfile() {
    return get().profiles.find((p2) => p2.isDefault) ?? null;
  },
  loadProfiles: async () => {
    var _a;
    ...
    try {
      ...
      const defaultProfile = profiles.find((p2) => p2.isDefault);
      set({
        profiles,
        isLoading: false,
        activeProfileId: (defaultProfile == null ? void 0 : defaultProfile.id) ?? ((_a = profiles[0]) == null ? void 0 : _a.id) ?? null
      });
    } catch (e) {
  ...
  updateProfile: async (id2, input) => {
    set({ error: null });
    try {
      const updated = await profileRepository.update(id2, input);
      if (updated) {
        set((s) => ({
          profiles: s.profiles.map((p2) => p2.id === id2 ? updated : p2)
        }));
      }
    } catch (e) {
      set({ error: String(e) });
  ...
  deleteProfile: async (id2) => {
    await profileRepository.delete(id2);
    set((s) => {
      var _a;
      const profiles = s.profiles.filter((p2) => p2.id !== id2);
      const activeProfileId = s.activeProfileId === id2 ? ((_a = profiles[0]) == null ? void 0 : _a.id) ?? null : s.activeProfileId;
      return { profiles, activeProfileId };
    });
  },
  setDefault: async (id2) => {
    await profileRepository.setDefault(id2);
    set((s) => ({
      profiles: s.profiles.map((p2) => ({ ...p2, isDefault: p2.id === id2 }))
    }));
  },
  setActiveProfile: (id2) => set({ activeProfileId: id2 }),
