...
class AppDatabase extends Xn {
  constructor() {
    ...
    __publicField(this, "formHistory");
...
const SENSITIVE_FIELDS = ["aadhaarNumber", "panNumber", "passportNumber", "drivingLicense"];
async function encryptIdentity(identity) {
...
async function decryptIdentity(identity) {
  const result = { ...identity };
  for (const key of SENSITIVE_FIELDS) {
    const val = result[key];
    if (typeof val === "string" && val) {
...
const profileRepository = {
  async getAll() {
    ...
    return Promise.all(records.map(async (r2) => {
      const { _id: _2, ...profile } = r2;
  ...
  async getById(id) {
    ...
    const { _id: _2, ...profile } = record;
    return { ...profile, identity: await decryptIdentity(profile.identity) };
  },
  async getDefault() {
    const record = await db.profiles.where("isDefault").equals(1).first();
    if (!record) return null;
    const { _id: _2, ...profile } = record;
    return { ...profile, identity: await decryptIdentity(profile.identity) };
  },
  ...
  async exportAll() {
    ...
    return JSON.stringify({ version: 1, exportedAt: (/* @__PURE__ */ new Date()).toISOString(), profiles }, null, 2);
  },
...
export {
  ...
  profileRepository as p,
  settingsRepository as s
};
